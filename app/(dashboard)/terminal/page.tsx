...
export default function TerminalPage() {
  ...
  const [mainMenu, setMainMenu] = useState("trading") // 'dashboard' | 'markets' | 'trading' | 'portfolio'
  ...
  useEffect(() => {
    ...
    wsDepth.onmessage = (event) => {
      const data = JSON.parse(event.data)
      let askTotal = 0;
      const processedAsks = (data.a || []).slice(0, 20).map((ask: any) => { askTotal += parseFloat(ask[1]); return [...ask, askTotal] }).reverse();
      let bidTotal = 0;
      const processedBids = (data.b || []).slice(0, 20).map((bid: any) => { bidTotal += parseFloat(bid[1]); return [...bid, bidTotal] });
  ...
  useEffect(() => {
    fetch('https://min-api.cryptocompare.com/data/v2/news/?lang=EN')
      .then(res => res.json())
      .then(data => { if (data && data.Data) setNewsList(data.Data.slice(0, 20)) })
  }, [])

  // FUNGSI HANDLE AI CHAT
  ...
  return (
    <div className="h-screen w-full bg-[#0b0e11] text-[#848e9c] font-mono overflow-hidden flex flex-col text-[11px] leading-tight select-none">
      ...
      <div className="h-8 border-b border-[#1e2329] bg-[#0b0e11] flex items-center justify-between px-2 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-4 text-[#848e9c] ml-4">
            <span onClick={() => setMainMenu('dashboard')} className={`cursor-pointer transition-colors ${mainMenu === 'dashboard' ? 'text-[#f59e0b] border-b-2 border-[#f59e0b] pb-[6px] font-bold' : 'hover:text-white'}`}>Dashboard</span>
            <span onClick={() => setMainMenu('markets')} className={`cursor-pointer transition-colors ${mainMenu === 'markets' ? 'text-[#f59e0b] border-b-2 border-[#f59e0b] pb-[6px] font-bold' : 'hover:text-white'}`}>Markets</span>
            <span onClick={() => setMainMenu('trading')} className={`cursor-pointer transition-colors ${mainMenu === 'trading' ? 'text-[#f59e0b] border-b-2 border-[#f59e0b] pb-[6px] font-bold' : 'hover:text-white'}`}>Crypto Trading</span>
            <span onClick={() => setMainMenu('portfolio')} className={`cursor-pointer transition-colors ${mainMenu === 'portfolio' ? 'text-[#f59e0b] border-b-2 border-[#f59e0b] pb-[6px] font-bold' : 'hover:text-white'}`}>Portfolio</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
      ...
      {/* RENDER KONDISIONAL BERDASARKAN MENU UTAMA */}
      {mainMenu !== 'trading' ? (
        <div className="flex-1 flex flex-col items-center justify-center bg-[#0b0e11] relative overflow-hidden">
          <div className="absolute inset-0 opacity-5 bg-[linear-gradient(#1e2329_1px,transparent_1px),linear-gradient(90deg,#1e2329_1px,transparent_1px)] bg-[size:40px_40px]"></div>
          <Activity size={48} className="text-[#f59e0b] mb-6 opacity-50" />
          <h1 className="text-3xl font-bold text-white tracking-widest uppercase mb-2">{mainMenu} MODULE</h1>
          <p className="text-[#848e9c] text-sm">Sistem ini sedang dalam tahap pengembangan struktur data.</p>
          <button onClick={() => setMainMenu('trading')} className="mt-8 border border-[#f59e0b] text-[#f59e0b] px-6 py-2 rounded hover:bg-[#f59e0b] hover:text-black transition-colors font-bold tracking-widest">
            KEMBALI KE CRYPTO TRADING
          </button>
        </div>
      ) : (
        <>
          {/* TERMINAL TRADING UTAMA */}
